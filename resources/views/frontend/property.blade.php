    ...
    <div class="container">
        ...
        <div class="row mb-3">
            <div class="col-xs-12 col-sm-6">
                @if(!\Auth::guest())
                <button class="selectProperty btn btn btn-primary"
                    data-url="{{ route('frontend.page', ['pageSlug' => 'property', 'locale' => \App::getLocale(), 'id' => $property->id]) }}">{{ __('Add to Watchlist') }}</button>
        ...
        <strong
            class="text-dark-blue">{{ $property->type->slug !== 'MORTGAGE-NOTE' ? __('Property') : __('Mortgage Note') }}</strong>
        <h2 class="m-0">{{ $property->type->slug !== 'MORTGAGE-NOTE' ? $property->address : $property->description }}
        </h2>
            ...
            <div class="row">
                <div class="col-md-8 p-0">
                    <div id="gallery" class="carousel slide" data-ride="carousel" data-interval="false">
                        <ol class="carousel-indicators">
                            ...
                            <li data-target="#gallery"
                                data-slide-to="{{ $property->youtube_video ? $index : $index - 1 }}"
                                class="@if($index == 1 && !$property->youtube_video) active @endif"></li>
                            @endforeach
                        </ol>
                    ...
                    <div class="text-danger">*
                        {{ __('The photos of the land are for illustrative purposes and do not necessarily represent the exact boundaries and fit of the property') }}
                    </div>
                    @endif
                ...
                <div class="col p-0 pl-sm-4 pr-sm-4 pt-sm-4">
                    ...
                    <div class="property-remaining" style="font-size: 13px;">
                        <vue-countdown-timer :start-time="{{$today->getTimestamp()}}"
                            :end-time="{{$endAt->getTimestamp()}}" :interval="1000" :start-label="'Until start:'"
                            :end-label="''" label-position="begin" :end-text="'Auction ended!'" :day-txt="'d'"
                            :hour-txt="'h'" :minutes-txt="'m'" :seconds-txt="'s'">
                        </vue-countdown-timer>
                        <br />
                        @if($online)
                        ...
                        <div class="alert alert-danger">
                            {{ __('Live Auction') }}:
                            <br />{{ Jenssegers\Date\Date::parse($property->event_live_at)->format('j M, g:ia')}}
                        </div>
                        @endif
                    </div>
                    @elseif($online)
                    ...
                    <div class=" mt-3">
                        ...
                        <br />
                        <strong>@if($property->buyer_prima){{ number_format(intval($property->buyer_prima)) }}%@else
                            1%@endif</strong>
                    </div>
                    ...
                    <div class="price mt-3">
                        ...
                        <br />
                        <strong class="unit">
                            <bid-component :user='{{\Auth::guest() ? 'A' : \Auth::user()->id}}'
                                :property='{{$property->id}}' :current='{{ intval(@$bid->offer ? $bid->offer : 0) }}'>
                            </bid-component>
                        </strong>

                        @if (!\Auth::guest() && $bid && $bid->user_id === \Auth::user()->id)<small
                            class="winning winning-{{\Auth::user()->id.$property->id}}">({{__('Winning')}})</small>
                        @endif
                    </div>
                    ...
                    <div class="price mt-3">
                        <strong class="text-dark-blue">{{ __('Make your offer') }} (No ",", only numbers, Minimum Bid:
                            ${{number_format($property->starting_bid)}})</strong>
                    </div>
                    @endif

                    @if($today->gte($biddingStartAt) && $today->lte($endAt) && ($online || ($property->status_id &&
                    !in_array($property->status->slug, ['OPTIONED', 'SOLD']))))
                    <div class="mt-3">
                        {!! form($form) !!}

                        <br />

                        @if (!Auth::guest() && (!$userEvent || $userEvent->remaining_deposit
                        <= 0)) {{ __('You must present your purchase intention by processing a minimum deposit') }}
                            <br />
        ...
        <gmap-map :center="{lat: {{ $property->latitude }}, lng: {{ $property->longitude }}}" :zoom="16"
            style="width:100%;  height: 300px;">
            <gmap-marker :position='{lat: {{ $property->latitude }}, lng: {{ $property->longitude }}}'></gmap-marker>
        </gmap-map>
        @endif
    </div>
</div>
