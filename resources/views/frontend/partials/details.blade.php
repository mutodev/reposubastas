    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-12 p-0 mb-3">
                ...
                <div>
                    <strong>{{ __('Original Loan Amount') }}:</strong>
                    <span>{{ $property->notes_original_loan_amount }}</span>
                </div>
                ...
                <div>
                    <strong>{{ __('Term/Length') }}:</strong> <span>{{ $property->notes_term }}</span>
                </div>
                ...
                <strong
                    class="text-danger">{{ __('***NO INSPECTION IS ALLOWED. THIS IS A MORTGAGE NOTE AUCTION AND DOES NOT REPRESENT COLLATERAL ADQUISITION. THIS COLLATERAL IS STILL PRIVATE PROPERTY AND SHOULD NOT BE TRESPASSED. IT IS REQUIRED TO SIGN A CONFIDENTIALITY AGREEMENT FOR MORE INFORMATION. 787-418-3100***') }}</strong>
            </div>
        </div>
        @endif
        <div class="row">
            <div class="col-12 p-0 col-md-4 mb-3">
                ...
                <div>
                    <strong>{{ __('CRIM ID') }}:</strong> <span><a href="https://www.satasgis.crimpr.net/cdprpc/"
                            target="_blank">{{ $property->catastro }}</a></span>
                </div>
                @endif
                @if($property->latitude && $property->longitude)
                <div>
                    <strong>{{ __('Coordinates') }}:</strong> <span><a
                            href="http://maps.google.com/maps?q={{ $property->latitude }},{{ $property->longitude }}"
                            target="_blank">{{ $property->latitude }}, {{ $property->longitude }}</a></span>
                </div>
                @endif
            ...
            <div class="col-12 p-0 col-md-4">
                ...
                <div class="mb-3">
                    ...
                    <span>{{ $property->is_cash_only ? __('Cash only') : __('Cash, Financed') }}</span>
                </div>
                ...
                <div>
                    <strong>{{ __('Live Auction') }}:</strong>
                    <span>{{ Jenssegers\Date\Date::parse($property->event_live_at)->format('j M, g:ia')}}</span>
                </div>
                @endif
            </div>
        </div>
