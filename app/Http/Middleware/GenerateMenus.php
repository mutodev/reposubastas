...
class GenerateMenus
{
    ...
    public function handle($request, Closure $next)
    {
        \Menu::make('SidebarMenu', function ($menu) use ($request) {
            ...
            $menu->add('<span title="icon envelope-open" aria-hidden="true" class="oi oi-envelope-open"></span> '.__('Deposits \ Bids'), ['url' => route('backend.users.deposits'), 'class' => 'nav-item'])->link->attr(['class' => 'nav-link']);
